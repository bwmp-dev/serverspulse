...
/**
 * Top-level [PlatformAdapter] for NeoForge servers on Minecraft 26.x.
 */
class NeoForgePlatformAdapter(
    ...
    private fun detectMcVersion(): String {
        return try {
            // WorldVersion is an interface with a `name()` accessor in 26.x,
            // not a bean-style getter, so this cannot be `.name` as on 1.21.x.
